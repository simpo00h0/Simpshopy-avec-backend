...
export function BlockSeparatorSettings({ customization, update }: BlockSettingsProps) {
  ...
  return (
    <Stack gap="sm">
      <Select label="Style" data={[{ value: 'line', label: 'Ligne' }, { value: 'dashed', label: 'Tirets' }, { value: 'dotted', label: 'Pointillé' }, { value: 'space', label: 'Espace' }]} value={sep?.style ?? 'line'} onChange={(v) => update('separator', { ...sep, style: (v as 'line' | 'space' | 'dotted' | 'dashed') ?? 'line' })} />
